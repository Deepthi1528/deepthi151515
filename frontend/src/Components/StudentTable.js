...
const StudentTable = () => { 
  ...
  const handleDeleteStudentClick = (regno) => { 
    if (window.confirm('Are you sure you want to delete this student?')) { 
      axios.delete(`http://localhost:8081/students/${regno}`) 
        .then(response => { 
    ...
    <div> 
      {!showAddStudent && !selectedStudent && !editingStudent ? ( 
        <> 
          ...
          <button className="add-button" onClick={handleAddStudentClick}>Add 
Student</button> 
...
handleViewStudentClick(student)}>View</button> 
                    <button className="action-btn edit" onClick={() => 
handleEditStudentClick(student)}>Edit</button> 
                    <button className="action-btn delete" onClick={() => 
handleDeleteStudentClick(student.regno)}>Delete</button> 
                  </td> 
                </tr> 
              ))} 
            </tbody> 
